namespace UnityEngine.PostProcessing
{
    public sealed class GrainComponent : PostProcessingComponentRenderTexture<GrainModel>
    {
        ...
        public override void Prepare(Material uberMaterial)
        {
            ...
            Material grainMaterial = this.context.materialFactory.Get("Hidden/Post FX/Grain Generator");
            grainMaterial.SetFloat(Uniforms._Phase, time / 20f);

            Graphics.Blit(null, this.m_GrainLookupRT, grainMaterial, settings.colored ? 1 : 0);
            ...
            uberMaterial.SetVector(Uniforms._Grain_Params2, new Vector4(this.context.width / (float)this.m_GrainLookupRT.width / settings.size, this.context.height / (float)this.m_GrainLookupRT.height / settings.size, rndOffsetX, rndOffsetY));
        }
    }
}
