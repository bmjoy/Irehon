...
namespace UnityEngine.PostProcessing
{
    ...
    public class PostProcessingBehaviour : MonoBehaviour
    {
        ...
        private void OnPreCull()
        {
            ...
            // Prepare context
            PostProcessingContext context = this.m_Context.Reset();
        ...
        private void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            if (taaActive)
            {
                RenderTexture tempRT = this.m_RenderTextureFactory.Get(src);
            ...
            Material fxaaMaterial = fxaaActive
            ...
            if (fxaaActive)
            {
                ...
                if (uberActive)
                {
                    RenderTexture output = this.m_RenderTextureFactory.Get(src);
        ...
        private void OnDisable()
        {
            ...
            foreach (KeyValuePair<CameraEvent, CommandBuffer> cb in this.m_CommandBuffers.Values)
            {
        ...
        {
            foreach (KeyValuePair<PostProcessingComponentBase, bool> cs in this.m_ComponentStates)
            {
            ...
            {
                PostProcessingComponentBase c = this.m_ComponentsToDisable[i];
            ...
            {
                PostProcessingComponentBase c = this.m_ComponentsToEnable[i];
            ...
            where T : PostProcessingModel
        {
            if (component.active)
            {
                CommandBuffer cb = this.GetCommandBuffer<T>(component.GetCameraEvent(), component.GetName());
                cb.Clear();
