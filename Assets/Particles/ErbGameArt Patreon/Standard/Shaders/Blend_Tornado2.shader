...
Shader "EGA/Particles/Blend_Tornado2" {
    Properties {
        ...
        _Cutoff ("Cutoff", Range(0, 0.5)) = 0.5
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_6450 = _Time;
                v.vertex.xyz += ((sin((_Numberofwawes*(mul( unity_WorldToObject, float4(mul(unity_ObjectToWorld, v.vertex).rgb,0) ).xyz.rgb.rgb.g+(node_6450.g*_Speedofwawes))*6.28318530718))*o.uv0.g)*v.normal*_VertexPower);
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float4 node_6731 = _Time;
                float2 node_1447 = (i.uv0+(node_6731.g*float2(_OpacityMaskSpeed.r,_OpacityMaskSpeed.g)));
                float4 _OpacityMask_var = tex2D(_OpacityMask,TRANSFORM_TEX(node_1447, _OpacityMask));
                float node_798 = (i.vertexColor.a*_TintColor.a*(i.uv0.b+(i.uv0.b*dot(_OpacityMask_var.rgb,float3(0.3,0.59,0.11)))));
                clip(node_798 - 0.5);
                float2 node_3204 = (i.uv0+(float2(_SpeedMainTexUVNoiseZW.b,_SpeedMainTexUVNoiseZW.a)*node_6731.g));
                float4 _Noise_var = tex2D(_Noise,TRANSFORM_TEX(node_3204, _Noise));
                float2 node_2085 = ((_Noise_var.r*_NoisePower)+(i.uv0+(node_6731.g*float2(_SpeedMainTexUVNoiseZW.r,_SpeedMainTexUVNoiseZW.g))));
